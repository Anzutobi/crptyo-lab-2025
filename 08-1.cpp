...
bign from_bytes_le(const uint8_t* buf,int len){
    bign res;
    res.len=0;
    uint64_t w0=0;
    uint64_t w1=0;
    for(int i=0;i<8 && i<len;++i) w0|=((uint64_t)buf[i]<<(8*i));
    for(int i=0;i<8 && i+8<len;++i) w1|=((uint64_t)buf[i+8]<<(8*i));
    res.d[0]=w0;
    res.d[1]=w1;
    if(w1!=0) res.len=2;
    else res.len=1;
...
struct MontPrecomp{
    bign table[1<<W_WINDOW];
};
MontPrecomp build_mont_precomp(const bign& a,const bign& n,uint64_t n_prime,const bign& R2_mod_n){
    MontPrecomp mp;
    bign a_mont=mont_mul(a,R2_mod_n,n,n_prime);
    mp.table[0]=mont_mul(one,R2_mod_n,n,n_prime);
    mp.table[1]=a_mont;
    for(int i=2;i<(1<<W_WINDOW);++i){
        mp.table[i]=mont_mul(mp.table[i-1],a_mont,n,n_prime);
    }
    return mp;
}
bign mont_pow_with_table(const MontPrecomp& mp,bign b,const bign& n,uint64_t n_prime){
    if(n.len==1 && n.d[0]==1) return zero;
    bign ans=mp.table[0];
    int bit_len=b.len*64-__builtin_clzll(b.d[b.len-1]);
    int i=bit_len-1;
    while(i>=0){
        if(((b.d[i/64]>>(i%64))&1)==0){
            ...
            for(int j=0;j<(i-s+1);++j) ans=mont_mul(ans,ans,n,n_prime);
            ans=mont_mul(ans,mp.table[value],n,n_prime);
            i=s-1;
        }
    }
    bign res=mont_mul(ans,one,n,n_prime);
    return res;
}
int main(){
	#ifdef _WIN32
	...
	//cout<<(int)data_len<<endl;
    bign p=from_bytes_le(p_buf,16);
    bign g=from_bytes_le(g_buf,16);
    bign s=from_bytes_le(s0_buf,16);
    //cout<<"p:"<<endl;
	//p.print();
    ...
    uint32_t cnt0=0;
    uint32_t cnt1=0;
    MontPrecomp mp_g=build_mont_precomp(g,p,n_prime,R2_mod_p);
    for(uint32_t i=0;i<data_len;++i){
        uint8_t byte_out=0;
        for(int bit=0;bit<8;++bit){
			s=mont_pow_with_table(mp_g,s,p,n_prime);
            int z=0;
            if(s<p_half){
                z=0;
                cnt0++;
