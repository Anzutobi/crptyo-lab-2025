...
// 使用 Brent 判环算法和 Teske's r-adding walk 优化 DLP 计算
bign rho(bign n,bign alpha,bign beta,bign p){
    ...
    while(1){
        ...
        bign x=x_mont;
        bign aa=a,bb=b,save_x=x,save_a=aa,save_b=bb;
        long long steps=0,limit=2;
        while(1){
            f(x,aa,bb);
            steps++;
            if(x==save_x){
                bign delta_b=bb-save_b;
                if(delta_b.sign) delta_b+=n;
                bign delta_a=save_a-aa;
