...
u64 kangaroo(u128 alpha,u128 beta){
    ...
    u64 mask=0xFF;
    u64 target=m_val/512;
    while(mask<target) mask=(mask<<1)|1;
    u64 xT=n_val;
    u128 yT=mont_pow(alpha,n_val);
    u128 yT_mont=mont_mul(yT,R2_mod_p);
    int limit=std::max((u64)(1.08*m_val),(u64)1114514);
    vector<pair<u128,u64>> table;
    table.reserve(4096);
    for(int i=0;i<limit;++i){
        int j=(int)(yT_mont & 31);
        xT=xT+S[j];
        yT_mont=mont_mul(yT_mont,M[j]);
        if((yT_mont & mask)==0){
            table.push_back({yT_mont, xT});
            if(table.size()>std::max((u64)16000,m_val/16)) break;
        }
    }
    ...
    for(int i=0;i<limit;++i){
        int j=(int)(yW_mont&31);
        xW=xW+S[j];
        yW_mont=mont_mul(yW_mont,M[j]);
        if((yW_mont&mask)==0){
            auto it=std::lower_bound(table.begin(),table.end(),make_pair(yW_mont,(u64)0));
            if(it!=table.end() && it->first==yW_mont){
                u64 target_x=it->second;
...
int main(){
    ...
    u64 res;
    if(n<(1ULL<<30)) res=rho(alpha,beta);
