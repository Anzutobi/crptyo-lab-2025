...
map<bign,bign> table;
bign kangaroo(bign n, bign a, bign b, bign p){
    uint64_t m_val;
    m_val=(uint64_t)sqrt((double)n.d[0]);
    bign m=(unsigned long long)m_val;
    if(m.len==1 && m.d[0]<100) m=100;
    uint64_t x=1;
    uint64_t n0=p.d[0];
    for(int i=0;i<6;++i) x=x*(2-n0*x);
    uint64_t p_prime=-x;
    ...
    for(int i=0;i<k;++i){
        S[i]=curr_s;
        if(S[i]>m) S[i]=m; 
        mont_mul(mont_pow(a,S[i],p),R2_mod_p,p,p_prime,M[i]);
        curr_s+=curr_s; 
    }
    uint64_t mask=0xFF;
    ...
    for(int i=0;i<limit;++i){
        ...
        if((yT_mont.d[0]&mask)==0){
            table[yT_mont]=xT;
            if(table.size()>std::max((uint64_t)4000,m_val/512)) break;
        }
    }
    bign xW=0;
    bign yW_mont;
...
int main(){
    ...
    if(n<1024) res=rho(n,a,b,p);
    else res=kangaroo(n,a%p,b%p,p);
    res.print();
    return 0;
}
