...
int main(){
	...
	for(int i=0;i<32;i+=8){
        ...
        while(_rdrand64_step(&t)==0) ;//wait for randrom
        memcpy(seed+i,&t,8);
    }
	uint8_t all[2048];
    fread(all,1,2048,fp);
    bign n=bign::from_bytes(all+16,256);
    bign e=bign::from_bytes(all+272,256);
    uint8_t mLen=all[784];
    uint8_t *M=all+785;
    int psLen=190-mLen;
    ...
    vector<uint8_t> seedMask=MGF(maskedDB.data(),dbLen,32),maskedSeed(32);
